    ...
    <div class="page_content_website" style="display: block;">
        ...
        <div class="contain_calamviec page_cty active" data-cty="7" style="display: block">
            <div class="form-content-add-ca" style="display: block;">
                <?php
                if (isset($_POST['add-phong-db'])) {
                    $tenPhong = $_POST['name_phong_ban'];
                    $ID_TP = $_POST['name_tp'];
                    $moTa = $_POST['mota_phong'];
                    $random_id_phong = rand(10000, 99999);
                    $sql_sel_phong = mysqli_query($conn, "SELECT name_phong FROM ql_phongban WHERE id_cty = 10000");
                    $row_all_phong = mysqli_fetch_assoc($sql_sel_phong);
                    if ($tenphong != $row_all_phong['name_phong']){
                        ...
                        $sql_insert_ca = mysqli_query($conn, "INSERT INTO ql_phongban(id_phong, name_phong, mota, truongphong, id_cty) values ('$random_id_phong', '$tenPhong', '$moTa', '$row_name_nv[name_nv]', 10000)");
                        $sql_upd_nv = mysqli_query($conn, "UPDATE user_nv SET id_phong = $random_id_phong, chucvu = 'Trưởng phòng' WHERE id_nv = $ID_TP AND id_cty = 10000");
                ...
                <form class="form-add-ca-st" action="" method="post">
                    <div class="form-add-ca">
                        <div class="add-name-ca">
                            ...
                            <input type="text" id="name_ca" name="name_phong_ban" required>
                        </div>
                        ...
                        <div class="add-mota">
                            ...
                            <input type="text" id="mota_phong" name="mota_phong" required>
                        </div>
                        <div class="submit-ca">
                            <input type="submit" name="add-phong-db" value="Thêm">
